...
class Convolution(Layer):
  ...
  def __init__(self, input_layer, num_filters, kernel_size, name, padding='SAME',
    ...
    if padding == 'SAME':
      # with zero padding
      self.shape = (N, num_filters, H, W) #50, 16, 28, 28
      self.pad = (kernel_size - 1) // 2 #2
    else:
      # without padding
      self.shape = (N, num_filters, H - kernel_size + 1, W - kernel_size + 1)
      self.pad = 0

    fan_in = C * kernel_size**2 # 1 * 5x5 = 25
...
class FC(Layer):
  ...
  def backward_inputs(self, grads):
    ...
    """
    N, num_outputs = grads.shape
    return grads.dot(self.weights.T).reshape(self.inputs.shape)
  ...
  def backward_params(self, grads):
    ...
    """
    N, num_outputs = grads.shape
    grad_weights = self.inputs.reshape(N, self.num_inputs).T.dot(grads)
...
class ReLU(Layer):
  ...
  def backward_inputs(self, grads):
    ...
    """
    gated = np.copy(grads)
    gated[np.where(self.inputs <= 0)] = 0.0
    return gated
...
class SoftmaxCrossEntropyWithLogits():
  ...
  def forward(self, x, y):
    ...
    return (np.log(np.sum(np.exp(x))) - np.sum(y*x)) / N
  ...
  def backward_inputs(self, x, y):
    ...
    softmax = lambda t: np.exp(t)/sum(np.exp(x))
    N, num_classes = x.shape
